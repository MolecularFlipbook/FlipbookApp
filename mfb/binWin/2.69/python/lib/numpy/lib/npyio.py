...
def genfromtxt(fname, dtype=float, comments='#', delimiter=None,
    ...
    * Individual values are not stripped of spaces by default.
    ...
    if isinstance(missing_values, (str, list, tuple)):
    ...
    if dtype is None:
        ...
        if len(dtype_flat) > 1:
            ...
            zipit = list(zip(dtype_flat, missing_values, filling_values))
            converters = [StringConverter(dt, locked=True,
                                          missing_values=miss, default=fill)
                           for (dt, miss, fill) in zipit]
        else:
            # Set to a default converter (but w/ different missing values)
            zipit = list(zip(missing_values, filling_values))
...
#                         for (convert, val) in zip(conversionfuncs, vals)])
    if loose:
        rows = list(zip(*[list(map(converter._loose_call, list(map(itemgetter(i), rows))))
                     for (i, converter) in enumerate(converters)]))
    else:
        rows = list(zip(*[list(map(converter._strict_call, list(map(itemgetter(i), rows))))
                     for (i, converter) in enumerate(converters)]))
    # Reset the dtype
    data = rows
    if dtype is None:
