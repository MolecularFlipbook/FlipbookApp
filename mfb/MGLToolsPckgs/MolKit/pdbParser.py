...
#
# $Header: /opt/cvs/python/packages/share1.5/MolKit/pdbParser.py,v 1.124.2.10 2012/10/02 16:35:06 sanner Exp $
#
# $Id: pdbParser.py,v 1.124.2.10 2012/10/02 16:35:06 sanner Exp $
#
...
##                                 line[71:76] ))
    ...
    def parse_PDB_Strand(self, lines):
        ...
        see: http://www.bmsc.washington.edu/CrystaLinks/man/pdb/part_44.html
        """
        sdatas = []
        for line in lines:
            try:
            ...
            if sense==0: # no registration info
...
#       if resSeq != mol.curRes.number or resName != mol.curRes.type:
        ...
        if use_split:
            ...
            atom._coords = [ [ float(orig_rec[30:38]), float(orig_rec[38:46]),
                               float(orig_rec[46:54]) ] ]
            atom._charges['pqr'] = float(orig_rec[55:62])
...
#        atom.conformation = 0
        ...
        if len(rec) >=76:
            charge = float(rec[70:76])
        else:
            charge = float(rec[55:61])
...
##          atom.segID = strip(rec[72:76])
        ...
        #atom.AtSolPar=float(strip(rec[78:84]))
        #atom.AtVol=float(strip(rec[86:92]))
        atom.AtVol = float(rec[78:84])
        atom.AtSolPar = float(rec[86:92])
