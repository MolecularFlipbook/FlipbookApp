...
class TextBlock(Widget):
	...
	def __init__(self, parent, name, text="", font=None, pt_size=None, color=None,
					size=[1, 1], pos=[0, 0], offset=[0,0], sub_theme='', overflow=BGUI_OVERFLOW_HIDDEN, options=BGUI_NONE):
		"""
	...
	def text(self, value):
		...
		cur_line = 0
		line = Label(self, "tmp", "Mj|", font=self._font, pt_size=self._pt_size, pos=[0,0], color=self._color, sub_theme=self.theme['LabelSubTheme'], options=BGUI_NORMALIZED|BGUI_NO_FOCUS)
		self._remove_widget(line)
		...
		linespacing = 1.3
		for words in lines:
			line = Label(self, "lines_"+str(cur_line), "", self._font, self._pt_size, self._color, pos=[0, 1-(cur_line+1)*char_height*linespacing], sub_theme=self.theme['LabelSubTheme'], options=BGUI_NORMALIZED|BGUI_NO_FOCUS)
		...
		if self.overflow:
			line_height = char_height*self.size[1]*linespacing
