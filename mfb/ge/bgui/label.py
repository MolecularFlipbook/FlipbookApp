...
class Label(Widget):
	...
	def _draw(self):
		...
		for i, txt in enumerate([i for i in self._text.split('\n')]):
			blf.position(self.fontid, self.position[0], self.position[1] - (self.size[1]*i*linespacing), 0)
			blf.draw(self.fontid, txt.replace('\t', '    '))
			self.dimension = list(blf.dimensions(self.fontid, txt.replace('\t', '    ')))[:]
