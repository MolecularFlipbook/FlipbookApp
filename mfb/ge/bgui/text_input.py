...
class TextInput(Widget):
	...
	def __init__(self, parent, name, text="", prefix="", font=None, pt_size=None, color=None,
					size=[0, 0], pos=[0, 0], offset=[0,0], sub_theme='', radius = 3, centerText=False, input_options=BGUI_INPUT_DEFAULT, options=BGUI_NONE):
		"""
		...
		self.frame = Frame(self, name+"_frame", size=[1,1], radius = radius, options = BGUI_NO_FOCUS | BGUI_NORMALIZED | BGUI_CENTERY)
		if centerText:
			self.highlight = Frame(self, name+"_highlight", size=self.frame.size, options = BGUI_NO_FOCUS | BGUI_CENTERED)
		else:
			self.highlight = Frame(self, name+"_highlight", size=self.frame.size, options = BGUI_NO_FOCUS | BGUI_CENTERY)
		self.highlight.border = 0
		if centerText:
			self.label = Label(self, name+"_label", text, font, pt_size, sub_theme=self.theme['LabelSubTheme'], options = BGUI_NO_FOCUS | BGUI_NORMALIZED | BGUI_CENTERX)
		else:
			self.label = Label(self, name+"_label", text, font, pt_size, sub_theme=self.theme['LabelSubTheme'], options = BGUI_NO_FOCUS | BGUI_NORMALIZED )
	
		self.cursor = Frame(self, name+"_cursor", options = BGUI_NO_FOCUS | BGUI_CENTERY)
		self.cursor.border = 0
	...
	def on_activate(self, value):
		self._on_activate = WeakMethod(value)

	@property
	def on_edit(self):
		"""A callback for when the edit is happening while the TextInput has focus"""
		return self._on_edit

	@on_edit.setter
	def on_edit(self, value):
		self._on_edit = WeakMethod(value)
	...
	def swapcolors(self, state = 0): #0 inactive 1 active
		...
		if state == 0:
			...
			self.cursor.visible = not self.centerText # only visible when centerText is not set
	...
	def _handle_mouse(self, pos, event):
		...
		if event == BGUI_MOUSE_LEFT_CLICK:
			...
			if self.just_activated or self._active and not self.input_options & BGUI_INPUT_SELECT_ALL:
				self.find_mouse_slice(pos)

		if event == BGUI_MOUSE_LEFT_RELEASE:
			...
			if self.click_counter == 0:
				...
				if time.time() - self.single_click_time < 0.4: #.2:
				...
				if time.time() - self.double_click_time < 0.2:
	...
	def _handle_key(self, key, is_shifted, is_ctrled):
		...
		if key == BACKSPACEKEY:
			...
			if self.on_backspace:
				self.on_backspace(self, text=self.label.text)
			...
			if key == ord(VKEY):
				chars = logic.keyboard.getClipboard().strip()
				self.label.text = self.text[:self.slice[0]] + chars + self.text[self.slice[1]:]

			if key == ord(CKEY):
				logic.keyboard.setClipboard(self.text)
	...
	def _draw(self):
		...
		if self.slice[0]-self.slice[1] == 0 and self._active:
			if time.time() - self.time > 1.0:
				...
				self.cursor.colors = [self.colors["text"][1]] *4
				# self.cursor.visible = True
				self.cursor.visible = not self.centerText
		else:
			self.cursor.visible = False
